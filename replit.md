...
- July 25, 2025: Fixed date format display in Invoice Management tables to show day/month/year format - updated both invoice-management.tsx and invoice-management-clean.tsx to use 'en-GB' locale with dd/mm/yyyy format instead of default browser locale; ensures consistent date display across all invoice tables showing dates like "27/06/2025" instead of "25/07/2025"
- July 25, 2025: Enhanced invoice date picker functionality with improved UX - replaced read-only date display in Edit Invoice with editable date input matching Create Invoice page; added enhanced styling with cursor pointer, prominent borders, hover effects, and focus states; implemented maximum date restriction to today preventing future date selection; both Create and Edit Invoice pages now have consistent date picker behavior with proper validation and backend integration
- July 19, 2025: Added comprehensive inline customer editing functionality to CustomerSearchDialog - implemented edit button for each customer with proper form validation using Zod schema; added edit customer dialog with name, place, phone, and customer type fields; included API integration with proper error handling and success notifications; applied same event prevention pattern as products to avoid parent form submission; customer edit functionality now available in both Create Invoice and Edit Invoice pages through shared CustomerSearchDialog component
- July 19, 2025: Fixed critical form submission bug in ProductSearchDialog - added proper event prevention (preventDefault, stopPropagation) and type="button" to all interactive buttons within the product search table and selected items section; resolved issue where clicking edit, add, remove, or quantity adjustment buttons was accidentally triggering the parent Create Invoice form submission and creating unwanted invoices
- July 14, 2025: Removed hardcoded default values in product management - changed expiry date default from "2025-12-31" to empty string and imageUrl default from placeholder URL to empty string in both add and edit product functions; now sends clean empty values instead of arbitrary defaults when fields are not provided
- July 14, 2025: Fixed missing shopId in product update API calls - added shopId extraction from JWT token decode in onEditProduct function; now both add and update product operations properly pass shopId to backend API; ensures products are correctly associated with the authenticated user's shop during updates
- July 14, 2025: Added amount paid input and balance calculation to create invoice page - added amount paid input field with balance calculation functionality to create invoice totals section; amount paid input shows placeholder of grand total value and dynamically calculates balance (grand total - amount paid) with color coding (red for positive balance, green for negative); enhanced existing auto-update functionality to default amount paid to grand total; verified amountPaid is properly passed to API in both create and edit invoice functions
- July 14, 2025: Fixed TypeScript compilation errors by adding address property to Shop interface - added optional address field to Shop interface in types/api.ts; updated edit-invoice.tsx inline shop type definitions to include address property; resolved 8 TypeScript errors across invoice-management.tsx, invoice-management-clean.tsx, InvoiceTemplate.tsx and edit-invoice.tsx files
- July 14, 2025: Increased blue header height from 140px to 180px across all invoice templates - updated create-invoice.tsx (3 functions), edit-invoice.tsx (2 functions), InvoiceTemplate component, invoice-management.tsx and invoice-management-clean.tsx for consistent header sizing; fixed customer data display in Invoice Management downloads to show actual customer phone and location from nested customer object instead of "N/A" values
- July 13, 2025: Completed comprehensive shop address integration across all invoice functions - added shop address and place information to all three PDF functions in create-invoice.tsx (preview and two download functions), both PDF functions in edit-invoice.tsx, and verified existing implementation in InvoiceTemplate component, invoice-management.tsx and invoice-management-clean.tsx; all invoice previews and downloads now display complete shop address details (address, place with location icon, GST, phone) in the header section
- June 30, 2025: Added shop address to Invoice Management PDF downloads - updated both invoice-management.tsx and invoice-management-clean.tsx PDF functions to include shop address and place information in downloaded invoices; shop header now displays complete address details matching the preview display
